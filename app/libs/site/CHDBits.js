...
class Site {
  constructor () {
    this.name = 'CHDBits';
    this.downloadLink = 'https://chdbits.co/download.php?id={ID}';
    this.url = 'https://chdbits.co/';
    this.id = 16;
  };
  ...
  async searchTorrent (keyword) {
    const torrentList = [];
    const document = await this._getDocument(`${this.index}torrents.php?incldead=0&spstate=0&inclbookmarked=0&search=${encodeURIComponent(keyword)}&search_area=${keyword.match(/tt\d+/) ? 4 : 0}&search_mode=0&tag=`);
