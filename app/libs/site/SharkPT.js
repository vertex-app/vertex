...
class Site {
  constructor () {
    this.name = 'SharkPT';
    this.downloadLink = 'https://sharkpt.net/download.php?id={ID}';
    this.url = 'https://sharkpt.net/';
  };

  async getInfo () {
    const info = {};
    const document = await this._getDocument(`${this.index}index.php`, false, 10);
    // 用户名
    info.username = document.querySelector('a[href*=userdetails] span').innerHTML;
    // uid
    info.uid = +document.querySelector('a[href*=userdetails]').href.match(/id=(\d+)/)[1];
    // 上传
    info.upload = document.querySelector('body > div.m_main > div.m_nav > div.m_nav_dropdowns > shark-popup > div > div:nth-child(6) > div.dropdown-menu-item-value').innerHTML.trim().replace(/(\w)B/, '$1iB');
    info.upload = util.calSize(...info.upload.split(' '));
    // 下载
    info.download = document.querySelector('body > div.m_main > div.m_nav > div.m_nav_dropdowns > shark-popup > div > div:nth-child(7) > div.dropdown-menu-item-value').innerHTML.trim().replace(/(\w)B/, '$1iB');
    info.download = util.calSize(...info.download.split(' '));
    // 做种
    info.seeding = +document.querySelector('body > div.m_main > div.m_nav > div.m_nav_dropdowns > shark-popup > div > div:nth-child(8) > div.dropdown-menu-item-value').innerHTML.trim();
    // 下载
    info.leeching = +document.querySelector('body > div.m_main > div.m_nav > div.m_nav_dropdowns > shark-popup > div > div:nth-child(9) > div.dropdown-menu-item-value').innerHTML.trim();
    // 做种体积
  ...
  async searchTorrent (keyword) {
    ...
    for (const _torrent of torrents) {
      ...
      console.info(torrent.title);
      torrent.subtitle = (_torrent.querySelector('.torrentname > tbody > tr .embedded > br+span:last-of-type') || _torrent.querySelector('.torrentname > tbody > tr .embedded > br')).nextSibling.nodeValue.trim();
