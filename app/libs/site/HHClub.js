...
class Site {
  ...
  async getInfo () {
    const info = {};
    const document = await this._getDocument('https://hhanclub.top/', false, 10);
    // 用户名
  ...
  async searchTorrent (keyword) {
    const torrentList = [];
    const document = await this._getDocument(`https://hhanclub.top/torrents.php?incldead=0&spstate=0&inclbookmarked=0&search=${encodeURIComponent(keyword)}&search_area=${keyword.match(/tt\d+/) ? 4 : 0}&search_mode=0&tag=`);
    ...
    for (const _torrent of torrents) {
      ...
      torrent.link = 'https://hhanclub.top/' + _torrent.querySelector('a[href*=details]').href.trim();
