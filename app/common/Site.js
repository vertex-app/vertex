...
class Site {
  ...
  async _searchMTeam (keyword) {
    ...
    if (this.adult) {
      torrentList.push(...(await this._searchMTeamAdult(keyword)).torrentList);
    }
    return {
      site: this.site,
      torrentList
  ...
  async _searchMTeamAdult (keyword) {
    const torrentList = [];
    const document = await this._getDocument(`https://kp.m-team.cc/adult.php?incldead=1&spstate=0&inclbookmarked=0&search=${encodeURIComponent(keyword)}&search_area=${keyword.match(/tt\d+/) ? 4 : 0}&search_mode=0`);
    const torrents = document.querySelectorAll('.torrents tbody tr:not(:first-child)');
    for (const _torrent of torrents) {
      const torrent = {};
      torrent.site = this.site;
  ...
  // HDDolby
  async _searchHDDolby (keyword) {
    const torrentList = [];
    const document = await this._getDocument(`https://www.hddolby.com/torrents.php?incldead=1&spstate=0&inclbookmarked=0&search=${encodeURIComponent(keyword)}&search_area=${keyword.match(/tt\d+/) ? 4 : 0}&search_mode=0`);
    const torrents = document.querySelectorAll('.torrents tbody tr:not(:first-child)');
    for (const _torrent of torrents) {
      const torrent = {};
      torrent.site = this.site;
  ...
  // HDChina
  async _searchHDChina (keyword) {
    const torrentList = [];
    const document = await this._getDocument(`https://hdchina.org/torrents.php?incldead=1&spstate=0&inclbookmarked=0&seeders=&search=${encodeURIComponent(keyword)}&search_area=${keyword.match(/tt\d+/) ? 4 : 0}&search_mode=0`);
    ...
    for (const _torrent of torrents) {
      ...
      torrent.category = _torrent.querySelector('td a[href*=cat] img').title.trim();
      torrent.link = 'https://hdchina.org/' + _torrent.querySelector('a[href*=details]').href.trim();
      torrent.downloadLink = 'https://hdchina.org/' + _torrent.querySelector('a[href*="download.php"]').href;
      torrent.id = +torrent.link.match(/id=(\d*)/)[1];
      torrent.seeders = +(_torrent.querySelector('a[href*=seeders] font') || _torrent.querySelector('a[href*=seeders]') || _torrent.querySelector('span[class=red]')).innerHTML.trim();
      torrent.leechers = +(_torrent.querySelector('a[href*=leechers]') || _torrent.childNodes[9]).innerHTML.trim();
      torrent.snatches = +(_torrent.querySelector('a[href*=snatches]') || _torrent.childNodes[11]).innerHTML.trim();
  ...
  // PTHome
  async _searchPTHome (keyword) {
    const torrentList = [];
    const document = await this._getDocument(`https://pthome.net/torrents.php?incldead=0&spstate=0&inclbookmarked=0&search=${encodeURIComponent(keyword)}&search_area=${keyword.match(/tt\d+/) ? 4 : 0}&search_mode=0&tag=`);
    const torrents = document.querySelectorAll('.torrents tbody tr:not(:first-child)');
    for (const _torrent of torrents) {
      const torrent = {};
      torrent.site = this.site;
      ...
      if (torrent.subtitle.indexOf('span') !== -1) torrent.subtitle = '';
      torrent.category = _torrent.querySelector('td a[href*=cat] img').title.trim();
      torrent.link = 'https://pthome.net/' + _torrent.querySelector('a[href*=details]').href.trim();
      torrent.id = +torrent.link.match(/id=(\d*)/)[1];
      torrent.seeders = +(_torrent.querySelector('a[href*=seeders] font') || _torrent.querySelector('a[href*=seeders]') || _torrent.querySelector('span[class=red]')).innerHTML.trim();
      torrent.leechers = +(_torrent.querySelector('a[href*=leechers]') || _torrent.childNodes[9]).innerHTML.trim();
      torrent.snatches = +(_torrent.querySelector('a[href*=snatches] b') || _torrent.childNodes[11]).innerHTML.trim();
  ...
  async _searchCHDBits (keyword) {
    ...
    for (const _torrent of torrents) {
      ...
      torrent.site = this.site;
      torrent.title = _torrent.querySelector('td[class="embedded"] > a[href*="details"]').title.trim();
      torrent.subtitle = (_torrent.querySelector('.torrentname > tbody > tr .embedded').lastChild.lastChild.nodeValue || '').trim();
      torrent.category = _torrent.querySelector('td a[href*=cat] img').title.trim();
      torrent.link = 'https://chdbits.co/' + _torrent.querySelector('a[href*=details]').href.trim();
      torrent.id = +torrent.link.match(/id=(\d*)/)[1];
      torrent.seeders = +(_torrent.querySelector('a[href*=seeders] font') || _torrent.querySelector('a[href*=seeders]') || _torrent.querySelector('span[class=red]')).innerHTML.trim();
      torrent.leechers = +(_torrent.querySelector('a[href*=leechers]') || _torrent.childNodes[9]).innerHTML.trim();
      torrent.snatches = +(_torrent.querySelector('a[href*=snatches] b') || _torrent.childNodes[11]).innerHTML.trim();
